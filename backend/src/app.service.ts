...
interface AdminCashSalePayload {
  fullName?: string;
  email?: string;
  phone?: string;
  packageId?: PackageId;
...
export class AppService {
  ...
  createAdminCashSale(authorization: string | undefined, payload: AdminCashSalePayload) {
    ...
    if (!fullName) {
      throw new BadRequestException('El nombre es obligatorio para registrar la venta.');
    }

    if (!phone) {
      throw new BadRequestException('El telefono es obligatorio para registrar la venta.');
    }
